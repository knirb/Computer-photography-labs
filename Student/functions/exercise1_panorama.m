...
pointsRef = points2d(:,:,REFERENCE_VIEW);
for i = 1:CAMERAS
   homographies(:,:,i) = compute_homography(pointsRef, points2d(:,:,i));
end
for c = 1:CAMERAS
...
%% Compute normalised homographies
% Determine all homographies to a reference view. We have:
% point in REFERENCE_VIEW = homographies(:,:,c) * point in image c.
% Remember, you have to set homographies{REFERENCE_VIEW} as well.
homographies = zeros(3,3,CAMERAS); 
...
for i = 1:CAMERAS
    points2d_norm(:,:,i) = norm_mat(:,:,i)*points2d(:,:,i);
end
pointsRef = points2d_norm(:,:,REFERENCE_VIEW);
for i = 1:CAMERAS
   homographies(:,:,i) = inv(norm_mat(:,:,REFERENCE_VIEW))*compute_homography(pointsRef, points2d_norm(:,:,i))*norm_mat(:,:,i);
end
